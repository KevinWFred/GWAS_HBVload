...
cmd=paste0(plink2," --bfile ",prefix," --pheno ../result/qHBsAg_pheno.txt --covar  ../result/sero_cov.txt --glm hide-covar --ci 0.95 --out ../result/qHBsAg")
system(cmd)
res=as.data.frame(fread("../result/qHBsAg.qHBsAg.glm.linear"))
...
png("../result/qHBsAg_qqplot.png",res=100)
qqplot(res$P) #1.03
dev.off()
library(qqman)
colnames(res)[1]="CHR"
png("../result/qHBsAg_manhattan.png",res=100, width=1200)
...
write.csv(tmp,file="../result/qHBsAg_sumstat.csv",row.names = F,quote=F)
