                  ...
                  HBeAg="all")
{
  ...
  HBeAgtable=read.table("../result/sero_hbeag_pheno.txt",header=T) #hbeag negative
  ...
  for(i in 1:ncol(snp.data)){
    ...
    if (HBeAg=="negative_B") #HBeAg negative and genotype B
    {
      cur.data=cur.data[rownames(cur.data) %in% intersect(HBeAgtable$IID,phenotypeB) ,]
    }
    if (HBeAg=="negative_C") #HBeAg negative and genotype C
    {
      cur.data=cur.data[rownames(cur.data) %in% intersect(HBeAgtable$IID,phenotypeC) ,]
    }
    
    if (HBeAg=="genotype_alt") #more covariates
    {
      cur.data=cur.data[rownames(cur.data) %in% c(phenotypeB,phenotypeC),]
      cur.data$HBVgenotype=factor(cur.data$HBVgenotype)
    }
    cur.data=cur.data[!is.na(cur.data$HBsAg_seroclearance),]
    
   if (HBeAg=="genotype_alt")
   {
     m1 = tryCatch(
       expr = {
         coxph(Surv(as.numeric(entry_date),as.numeric(sc_date), HBsAg_seroclearance) ~  HBVgenotype+ALT+AGE.grp + SEX + PC1 + PC2 + PC3 + PC4 + PC5+ SNP, cur.data)
       },
       error = function(e){ 
         return(NULL)
       }
...
print("done")
allfiles=list.files("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/","*.gz")
# tmp=data.frame(code=rep("/data/BB_Bioinformatics/Kevin/HBV_GWAS/code/Cox_seroclearance_GWAS_maf01.R",length(allfiles)),
#                snptrawfile=paste0("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/",allfiles),
#                outfile=paste0("../result/cox_sero_maf01/",gsub(".gz",".txt",allfiles,fixed=T)))
# write.table(tmp,file="log/cox_sero_maf01.swarm",row.names = F,col.names = F,sep="\t",quote=F)
#9013095
#swarm -f /data/BB_Bioinformatics/Kevin/HBV_GWAS/code/log/cox_sero_maf01.swarm -g 16 --module R/4.2.0 --time=10:00:00

#hbeag negative
# allfiles=list.files("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/","*.gz")
# tmp=data.frame(code=rep("/data/BB_Bioinformatics/Kevin/HBV_GWAS/code/Cox_seroclearance_GWAS_maf01.R",length(allfiles)),
#                snptrawfile=paste0("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/",allfiles),
#                outfile=paste0("../result/cox_sero_hbeag_neg_maf01/",gsub(".gz",".txt",allfiles,fixed=T)),
#                hbeag="negative")
# write.table(tmp,file="log/cox_sero_hbeag_neg_maf01.swarm",row.names = F,col.names = F,sep="\t",quote=F)
#9013255
#swarm -f /data/BB_Bioinformatics/Kevin/HBV_GWAS/code/log/cox_sero_hbeag_neg_maf01.swarm -g 16 --module R/4.2.0 --time=10:00:00

#hbeag negative, typeB
# allfiles=list.files("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/","*.gz")
# tmp=data.frame(code=rep("/data/BB_Bioinformatics/Kevin/HBV_GWAS/code/Cox_seroclearance_GWAS_maf01.R",length(allfiles)),
#                snptrawfile=paste0("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/",allfiles),
#                outfile=paste0("../result/cox_sero_hbeag_neg_typeB_maf01/",gsub(".gz",".txt",allfiles,fixed=T)),
#                hbeag="negative_B")
# write.table(tmp,file="log/cox_sero_hbeag_neg_typeB_maf01.swarm",row.names = F,col.names = F,sep="\t",quote=F)
#9099032
#swarm -f /data/BB_Bioinformatics/Kevin/HBV_GWAS/code/log/cox_sero_hbeag_neg_typeB_maf01.swarm -g 16 --module R/4.2.0 --time=12:00:00

#hbeag negative, typeC
# allfiles=list.files("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/","*.gz")
# tmp=data.frame(code=rep("/data/BB_Bioinformatics/Kevin/HBV_GWAS/code/Cox_seroclearance_GWAS_maf01.R",length(allfiles)),
#                snptrawfile=paste0("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/",allfiles),
#                outfile=paste0("../result/cox_sero_hbeag_neg_typeC_maf01/",gsub(".gz",".txt",allfiles,fixed=T)),
#                hbeag="negative_C")
# write.table(tmp,file="log/cox_sero_hbeag_neg_typeC_maf01.swarm",row.names = F,col.names = F,sep="\t",quote=F)
#9099889
#swarm -f /data/BB_Bioinformatics/Kevin/HBV_GWAS/code/log/cox_sero_hbeag_neg_typeC_maf01.swarm -g 16 --module R/4.2.0 --time=12:00:00
#type and alt
# allfiles=list.files("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/","*.gz")
# tmp=data.frame(code=rep("/data/BB_Bioinformatics/Kevin/HBV_GWAS/code/Cox_seroclearance_GWAS_maf01.R",length(allfiles)),
#                snptrawfile=paste0("/data/DCEGLeiSongData/Kevin/HBVloadGwas/splited_maf01/",allfiles),
#                outfile=paste0("../result/cox_sero_type_alt_maf01/",gsub(".gz",".txt",allfiles,fixed=T)),
#                hbeag="negative_alt")
# write.table(tmp,file="log/cox_sero_type_alt_maf01.swarm",row.names = F,col.names = F,sep="\t",quote=F)
#9099901
#swarm -f /data/BB_Bioinformatics/Kevin/HBV_GWAS/code/log/cox_sero_type_alt_maf01.swarm -g 16 --module R/4.2.0 --time=12:00:00